...
BOOST_AUTO_TEST_CASE(TwoActivatedMusclesImplicitFeedback)
{
  ...
  for (int timestep = 0; timestep < 2; ++timestep) {
    ...
    if (context.isNamed("M1SM")) {
      ...
      participant.readData( "Stretch_M1_Mesh","StretchM1SM", stretchVertexIDs, timestepSize, receivedStretch1 );

    } else {

      BOOST_TEST(context.isNamed("M2"));
      participant.writeData("Activation_M2_Mesh","Activation2", activationVertexIDs , activation2 );
      participant.readData( "Stretch_M2_Mesh","StretchM2SM", stretchVertexIDs, timestepSize, receivedStretch2 );
